...
int edittab::m_initial_zoom = 2;
int edittab::m_initial_snap = c_ppqn / 4;
int edittab::m_initial_note_length = c_ppqn / 4;
int edittab::m_initial_scale = 0;
int edittab::m_initial_key = 0;
int edittab::m_initial_sequence = -1;
...
void
edittab::menu_action_quantise()
{

}

edittab::edittab( sequence *a_seq,
          perform *a_perf,
          // mainwid *a_mainwid,
          int a_pos  ) :
    /* set the performance */
    ...
    m_sequence(m_initial_sequence)
{
    ...
    m_button_data->signal_clicked().connect(
            mem_fun( *this, &edittab::popup_event_menu));
    ...
    m_toggle_play->signal_clicked().connect(
            mem_fun( *this, &edittab::play_change_callback));
    ...
    m_toggle_record->signal_clicked().connect(
            mem_fun( *this, &edittab::record_change_callback));
    ...
    m_toggle_q_rec->signal_clicked().connect(
            mem_fun( *this, &edittab::q_rec_change_callback));
    ...
    m_button_rec_vol->signal_clicked().connect(
            sigc::bind<Menu *>( mem_fun( *this, &edittab::popup_menu), m_menu_rec_vol  ));
    ...
    m_toggle_thru->signal_clicked().connect(
            mem_fun( *this, &edittab::thru_change_callback));
...
edittab::create_menus()
{
    ...
    for (int i = c_min_zoom; i <= c_max_zoom; i*=2)
    {
        ...
        m_menu_zoom->items().push_back(MenuElem(b,
                    sigc::bind(mem_fun(*this, &edittab::set_zoom), i )));
    }

    /* note snap */
    m_menu_snap->items().push_back(MenuElem("1",
                sigc::bind(mem_fun(*this, &edittab::set_snap),
                    c_ppqn * 4  )));
    ...
    m_menu_note_length->items().push_back(MenuElem("1",
                sigc::bind(mem_fun(*this, &edittab::set_note_length),
                    c_ppqn * 4  )));
    ...
    for( int i=0; i<16; i++ ){
        ...
        m_menu_length->items().push_back(MenuElem(b,
                    sigc::bind(mem_fun(*this, &edittab::set_measures), i+1 )));
        /* length */
        m_menu_bpm->items().push_back(MenuElem(b,
                    sigc::bind(mem_fun(*this, &edittab::set_bpm), i+1 )));
    }

    m_menu_length->items().push_back(MenuElem("32",
                sigc::bind(mem_fun(*this, &edittab::set_measures), 32 )));
    m_menu_length->items().push_back(MenuElem("64",
                sigc::bind(mem_fun(*this, &edittab::set_measures), 64 )));
...
edittab::popup_tool_menu()
{
    ...
    holder->items().push_back( MenuElem( "All Notes",
                sigc::bind(mem_fun(*this, &edittab::do_action),
                    select_all_notes, 0)));
...
void
edittab::do_action( int a_action, int a_var )
{
    switch (a_action) {
...
edittab::fill_top_bar()
{
    ...
    m_entry_name->signal_changed().connect(
            mem_fun( *this, &edittab::name_change_callback));
    ...
    m_button_bpm->signal_clicked().connect(
            sigc::bind<Menu *>( mem_fun( *this, &edittab::popup_menu),
                m_menu_bpm  ));
    ...
    m_button_bus->signal_clicked().connect(
            mem_fun( *this, &edittab::popup_midibus_menu));
    ...
    m_button_channel->signal_clicked().connect(
            mem_fun( *this, &edittab::popup_midich_menu ));
    ...
    m_button_undo->signal_clicked().connect(
            mem_fun( *this, &edittab::undo_callback));
    ...
    m_button_redo->signal_clicked().connect(
            mem_fun( *this, &edittab::redo_callback));
    ...
    m_button_quanize->signal_clicked().connect(
            sigc::bind(mem_fun(*this, &edittab::do_action), quantize_notes, 0));
    ...
    m_button_tools->signal_clicked().connect(
            mem_fun( *this, &edittab::popup_tool_menu ));
    ...
    m_button_sequence->signal_clicked().connect(
            mem_fun( *this, &edittab::popup_sequence_menu));
    ...
    m_radio_select->signal_clicked().connect( sigc::bind(mem_fun( *this, &edittab::mouse_action ), e_action_select ));
    m_hbox3->pack_start( *m_radio_select, false, false );

    /* Draw */
    m_radio_draw = manage( new RadioButton( "Draw" ));
    m_radio_draw->signal_clicked().connect( sigc::bind(mem_fun( *this, &edittab::mouse_action ), e_action_draw ));
    m_hbox3->pack_start( *m_radio_draw, false, false );

    /* Grow */
    m_radio_grow = manage( new RadioButton( "Grow" ));
    m_radio_grow->signal_clicked().connect( sigc::bind(mem_fun( *this, &edittab::mouse_action ), e_action_grow ));
...
#if 0
void
edittab::mouse_action( mouse_action_e a_action )
{
...
void
edittab::popup_menu(Menu *a_menu)
{
    a_menu->popup(0,0);
}
...
edittab::popup_midibus_menu()
{
    ...
    for ( int i=0; i< masterbus->get_num_out_buses(); i++ ){
        m_menu_midibus->items().push_back(MenuElem(
                    ...
                    sigc::bind(mem_fun(*this,&edittab::set_midi_bus), i)));
    }
...
edittab::popup_midich_menu()
{
    ...
    for( int i=0; i<16; i++ ){
        ...
        m_menu_midich->items().push_back(MenuElem(name,
                    sigc::bind(mem_fun(*this,&edittab::set_midi_channel),
                                                       i )));
    }
...
edittab::popup_sequence_menu()
{
    ...
    m_menu_sequences->items().push_back(MenuElem("Off",
                sigc::bind(mem_fun(*this, &edittab::set_background_sequence), -1)));
        ...
        for ( int seq=0; seq<  c_seqs_in_set; seq++ ){
            ...
            if ( m_mainperf->is_active( i )){
                ...
                menu_ss->items().push_back(MenuElem(name,
                            sigc::bind(mem_fun(*this, &edittab::set_background_sequence), i)));
...
void
edittab::set_background_sequence( int a_seq )
{
    char name[30];
...
Gtk::Image*
edittab::create_menu_image( bool a_state )
{
    if ( a_state )
...
edittab::popup_event_menu()
{
    ...
    m_menu_data->items().push_back( ImageMenuElem( "Note On Velocity",
                *create_menu_image( note_on ),
                sigc::bind(mem_fun(*this, &edittab::set_data_type),
...
void
edittab::set_midi_channel( int a_midichannel  )
{
    char b[10];
...
void
edittab::set_midi_bus( int a_midibus )
{
...
void
edittab::set_note_length( int a_note_length  )
{
    char b[10];
...
void
edittab::apply_length( int a_bpm, int a_bw, int a_measures )
{
...
void
edittab::set_measures( int a_length_measures  )
{
    char b[10];
...
void
edittab::set_bpm( int a_beats_per_measure )
{
    char b[4];
...
void
edittab::set_bw( int a_beat_width  )
{
    char b[4];
...
void
edittab::set_rec_vol( int a_rec_vol  )
{
...
void
edittab::record_change_callback()
{
...
void
edittab::set_data_type( unsigned char a_status, unsigned char a_control  )
{
...
bool
edittab::on_delete_event(GdkEventAny *a_event)
{
    //printf( "edittab::on_delete_event()\n" );
...
bool
edittab::on_scroll_event( GdkEventScroll* a_ev )
{
    //printf("edittab::on_scroll_event(x=%f,y=%f,state=%d)\n", a_ev->x, a_ev->y, a_ev->state);
...
bool
edittab::on_key_press_event( GdkEventKey* a_ev )
{
    guint modifiers;    // Used to filter out caps/num lock etc.
    modifiers = gtk_accelerator_get_default_mod_mask ();
