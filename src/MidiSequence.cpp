...
void
MidiSequence::set_song_mute( bool a_mute )
{
    m_song_mute = a_mute;
}

bool
MidiSequence::get_song_mute()
{
    return m_song_mute;
}
...
void
MidiSequence::set_rec_vol( long a_rec_vol )
{
    lock();
    m_rec_vol = a_rec_vol;
    unlock();
...
bool MidiSequence::get_song_playback_block(){
    return m_song_playback_block;
}

void MidiSequence::set_song_playback_block(bool a_block){
    m_song_playback_block = a_block;
}

bool MidiSequence::get_song_recording(){
...
void
MidiSequence::set_orig_tick( long a_tick )
{
    lock();
    m_last_tick = a_tick;
    unlock();
...
bool
MidiSequence::get_queued()
{
    return m_queued;
}

long
MidiSequence::get_queued_tick()
{
...
int
MidiSequence::get_num_selected_notes( )
{
    int ret = 0;
...
void
MidiSequence::increment_selected( unsigned char a_status, unsigned char a_control )
{
    lock();
...
void
MidiSequence::decrement_selected(unsigned char a_status, unsigned char a_control )
{
    lock();
...
void
MidiSequence::play_note_on( int a_note )
{
    lock();
...
/* plays a note from the paino roll */
void
MidiSequence::play_note_off( int a_note )
{
    lock();
...
bool MidiSequence::intersectTriggers( long position, long& start, long& end )
{
    lock();
...
bool MidiSequence::intersectNotes( long position, long position_note, long& start, long& end, long& note )
{
    lock();
...
bool MidiSequence::intersectEvents( long posstart, long posend, long status, long& start )
{
    lock();
...
void
MidiSequence::grow_trigger (long a_tick_from, long a_tick_to, long a_length)
{
    lock();
...
void
MidiSequence::del_trigger( long a_tick )
{
    lock();
...
void
MidiSequence::set_trigger_offset( long a_trigger_offset )
{
    lock();
...
long
MidiSequence::get_trigger_offset()
{
    return m_trigger_offset;
}

void
MidiSequence::split_trigger( MidiTrigger &trig, long a_split_tick)
{
    lock();
...
void
MidiSequence::adjust_trigger_offsets_to_legnth( long a_new_len )
{
    lock();
...
void
MidiSequence::half_split_trigger( long a_tick )
{
    lock();
...
void
MidiSequence::exact_split_trigger(long a_tick){
...
long
MidiSequence::get_selected_trigger_start_tick()
{
    long ret = -1;
    lock();
...
long
MidiSequence::get_selected_trigger_end_tick()
{
    long ret = -1;
    lock();
...
long
MidiSequence::adjust_offset( long a_offset )
{
    a_offset %= m_length;
...
bool
MidiSequence::get_trigger_state( long a_tick )
{
    lock();
...
bool
MidiSequence::select_trigger( long a_tick )
{
    lock();
...
bool
MidiSequence::unselect_triggers()
{
    lock();
...
void
MidiSequence::copy_selected_trigger()
{
    lock();
...
void
MidiSequence::reset_draw_marker()
{
    lock();
...
void
MidiSequence::reset_draw_trigger_marker()
{
    lock();
...
int
MidiSequence::get_lowest_note_event()
{
    lock();
...
int
MidiSequence::get_highest_note_event()
{
    lock();
...
bool
MidiSequence::get_next_trigger( long *a_tick_on, long *a_tick_off, bool *a_selected, long *a_offset )
{
...
const char*
MidiSequence::get_name()
{
    return m_name.c_str();
}

long
MidiSequence::get_last_tick( )
{
    return (m_last_tick + (m_length -  m_trigger_offset)) % m_length;
}

void
MidiSequence::set_midi_bus( char  a_mb )
{
    lock();
...
void
MidiSequence::set_length( long a_len, bool a_adjust_triggers )
{
    lock();
...
void
MidiSequence::set_playing( bool a_p )
{
    lock();
...
void
MidiSequence::set_recording( bool a_r )
{
    lock();
    m_recording = a_r;
    m_notes_on = 0;
...
bool
MidiSequence::get_recording( )
{
    return m_recording;
}

void
MidiSequence::set_snap_tick( int a_st )
{
    lock();
    m_snap_tick = a_st;
    unlock();
...
void
MidiSequence::set_quanized_rec( bool a_qr )
{
    lock();
    m_quanized_rec = a_qr;
    unlock();
...
bool
MidiSequence::get_quanidez_rec( )
{
    return m_quanized_rec;
}

void
MidiSequence::set_thru( bool a_r )
{
    lock();
    m_thru = a_r;
    unlock();
...
/* sets sequence name */
void
MidiSequence::set_name( char *a_name )
{
    m_name = a_name;
    set_dirty_mp();
}
...
void
MidiSequence::set_name( string a_name )
{
    m_name = a_name;
    set_dirty_mp();
}
...
void
MidiSequence::set_midi_channel( unsigned char a_ch )
{
    lock();
    off_playing_notes( );
    m_midi_channel = a_ch;
...
void
MidiSequence::print_triggers()
{
    printf("[%s]\n", m_name.c_str()  );

    for( list<MidiTrigger>::iterator i = m_list_trigger.begin();
...
void
MidiSequence::off_playing_notes()
{
    lock();
...
/* change */
void
MidiSequence::select_events( unsigned char a_status, unsigned char a_cc, bool a_inverse )
{
    lock();
...
void
MidiSequence::fill_list( list<char> *a_list, int a_pos )
{
    ...
    *a_list = list<char>();
    ...
    list<MidiTrigger>::iterator t = m_list_trigger.begin();
    list<MidiTrigger>::iterator p;
